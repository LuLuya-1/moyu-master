...
### 使用 Cloudflare 临时公网地址

如果不想绑定托管平台的信用卡，可以用 Cloudflare Quick Tunnel 临时邀请朋友试玩。先在第一个终端运行：
...
第二个终端会显示一个以 `https://` 开头、以 `trycloudflare.com` 结尾的临时地址。把该地址发给朋友即可。两个终端或电脑关闭后，临时地址会失效；下次启动通常会生成新地址。
...
每名玩家的回合包含两个阶段：先购买一张牌或放弃购买，然后必须弃掉市场上一张牌。弃牌完成后服务器自动轮到下一人。玩家掉线时回合会停在原处等待重连，不会自动跳过。
